...
def maintenance_api():
    ...
    try:
        ...
        log_query(user, 'maintenance', query, ai_result['data']['response'] if ai_result['success'] else error_str, ai_result['success'])
        return {
        ...
        log_query(user, 'maintenance', query, str(e), False)
        return {
            'success': False,
            'data': None,
            'error': str(e)
